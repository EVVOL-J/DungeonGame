...
public class DungeonGame extends Game {
    ...
    // +Пропуск хода сделал по нажатию пробела.3. На соответствующие действия персонаж тратит эти очки, если сделать ничего не может/не хочет, то
    ...
    // Создал новый класс Money и MoneyController, при убийстве монеты появляются в клетке умершего в рандомных координатах.
    //Если кто-то наступил в ячейку ,где находятся монеты, то монеты передаются наступившему.
    // +5. * Монеты высыпаются на пол, и чтобы их забрать, надо на них наступить.
    // В GameMap создал массив где проверяю активирована ли клетка героем, если нет то туман. Поле зрения 5 клеток.
